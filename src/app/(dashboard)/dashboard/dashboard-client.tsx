...
const AddBookmarkModal = dynamic(
  () => import("@/features/bookmarks/dialogs/add-bookmark-modal").then((mod) => mod.AddBookmarkModal),
  { ssr: false }
);

const BookmarkGrid = dynamic(
  () => import("@/features/bookmarks/components/bookmark-grid").then((mod) => mod.BookmarkGrid),
  { loading: () => <Skeleton className="h-40 w-full" /> }
);

export function DashboardClient({ userId: serverUserId }: { userId?: string }) {
  const { isLoaded, isSignedIn, user } = useUser();
  const router = useRouter();
  
  // Prefer server-provided userId if available, otherwise fallback to client state
  const userId = serverUserId || user?.id;

  // Real-time PC ↔ Mobile sync listener
  useBookmarksRealtime(userId);

  // Optimistic UI custom hook for instant local updates & Drizzle ORM background sync
  // With HydrationBoundary, this will return cached data instantly
  const {
  ...
  const { data: queryCategories } = useCategoriesQuery(true); // Now always enabled since server handles auth
  ...
  // With RSC & Hydration, this loading state should rarely be seen
